            ...
            case svc_playerinfo:
            {
                ...
                for (int i = 0; i < 3; i++) {
                    if (flags & (PF_VELOCITY1 << i)) {
                        cout << "PF_VELOCITY" << i << "(" << num << ")" << "     = " << message->readShort() << endl;
                    }
                }
                ...
                if (flags & PF_SKINNUM) {
                    cout << "PF_SKINNUM(" << num << ")" << "       = " << message->readByte() << endl;
                }

                if (flags & PF_EFFECTS) {
                    cout << "PF_EFFECTS(" << num << ")" << "       = " << message->readByte() << endl;
                }

                if (flags & PF_WEAPONFRAME) {
                    cout << "PF_WEAPONFRAME(" << num << ")" << " = " << message->readByte() << endl;
                }
...
void Bot::think() {
    ...
    float mx = me->coords[0];
    float mz = me->coords[1];
    float my = me->coords[2];
...
void Bot::epoch() {
    ...
    Genome baby1, baby2;
    crossoverAtSplits(mom.getGenes(), dad.getGenes(), baby1.getGenes(), baby2.getGenes());
...
Genome Bot::randomSelection() {
    ...
    cout << "ANN selected random genome with fitness of " << selected.getFitness() << endl;
...
Genome Bot::wheelSelection() {
    ...
    cout << "ANN selected genome with fitness of " << selected.getFitness() << " slice = " << slice << endl;
...
void Bot::crossoverAtSplits(const vector<double> &mom, const vector<double> &dad, vector<double> &baby1, vector<double> &baby2) {
